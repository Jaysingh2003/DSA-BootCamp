...
public class Cyclicsort {
    ...
    static void cyclicsort(int[] arr) {
        int i = 0;
        while (i < arr.length) {
            int correct = arr[i] - 1;//the corrcet will store the index no the value
            if (arr[i] != arr[correct]) {//if the value of the corrcet index  and index are (npte we can swap the value no the index )
                swap(arr, i, correct);
            } else {
                i++;
            }
